...
def check_lib(lib, unique_words, word_to_books, sort_output=True):
    ...
    if sk == sorted(word_to_books[word]):
        ...
        print(sk)
        print(sorted(word_to_books[word]))
...
def main():
    ...
    texts = [["The", "name", "of", "this", "book", "contains", "a", "number","The"],
    ...
    # Corrected loop to populate word_to_books
    for book, text in zip(book_titles, texts):
        for word in text:  # Fix: Iterate over individual words in the text
